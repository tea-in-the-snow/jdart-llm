...
public class INVOKEVIRTUAL extends gov.nasa.jpf.jvm.bytecode.INVOKEVIRTUAL {
  ...
  public Instruction execute(ThreadInfo ti) {
    ...
    if (objAttr instanceof Expression) {
      ...
      if (callee != null) {
        ...
        // Get the static declared type (the class where the method is declared)
        ...
        if (!possibleTypes.isEmpty()) {
          ...
          if (isFirstVisit) {
            System.out.println("\n\n++++++++++++++++++++++++++++++++++++++++++++++");
            System.out.println("INVOKEVIRTUAL: collecting constraints for method call to " + mname);
            ...
            System.out.println("Generated instanceof constraints for INVOKEVIRTUAL:");
          ...
          if (isFirstVisit && actualType != null) {
            System.out.println(
                "INVOKEVIRTUAL instanceof constraints: " + possibleTypes.size() +
  ...
  * 
  * @param ti              ThreadInfo
  * @param declaredType    The type where the method is declared
  * @param actualType      The runtime type observed during the current execution
    ...
    if (discoveredClasses.isEmpty()) {
      try {
        ...
        for (gov.nasa.jpf.vm.ClassLoaderInfo classLoader : classLoaderList) {
          if (classLoader != null) {
            scanClassLoaderForSubclasses(classLoader, declaredType, methodName, methodSignature, discoveredClasses);
          }
        }
      } catch (Exception e) {
        System.err.println(
  ...
  /**
   * Scan a class loader for all loaded classes that are subclasses of the base
   * class
   * and have a concrete implementation of the specified method.
   * 
   * @param classLoader       The class loader to scan
   * @param baseClass         The base class to look for subclasses of
   * @param methodName        Method name
   * @param methodSignature   Method signature
   * @param discoveredClasses Set to add discovered classes to
   */
  private void scanClassLoaderForSubclasses(
    ...
    for (ClassInfo ci : classLoader) {
      if (ci != null && isSubclassOf(ci, baseClass)) {
   ...
   * @return true if candidate is a subclass of baseClass
   */
